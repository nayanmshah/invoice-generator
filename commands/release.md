---
description: "Create a new release of the Nayan extension"
...
7. If a major or minor release:
    ...
    - Use the new_task tool to create a subtask in `code` mode (skillHint: nayan-translation) with detailed instructions of which content needs to be translated into all supported languages (The READMEs as well as the translation strings)
8. Create a new branch for the release preparation: `git checkout -b release/v[version]`
9. **Single-repo:** Run from workspace root. **Multi-repo:** If this workspace is multi-repo, run from the repo that contains `.changeset/` and release artifacts (usually not applicable for Nayan extension). Commit and push the changeset file and any documentation updates to the repository: `git add . && git commit -m "chore: add changeset for v[version]" && git push origin release/v[version]`
